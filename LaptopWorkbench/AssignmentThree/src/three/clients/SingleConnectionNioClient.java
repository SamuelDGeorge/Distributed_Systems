...
public abstract class SingleConnectionNioClient implements Runnable {
	...
	public synchronized void sendData(byte[] data) throws IOException {
		if (this.connected) {
			...
			synchronized (this.pendingData) {
				...
				//Wrap in a comma at the end in case multiple writes are read by the 
				//server. This will allow them to be deserialized and read correctly.
				String toWrap = new String(data);
				toWrap = toWrap + ",";
				queue.add(ByteBuffer.wrap(toWrap.getBytes()));
	...
	protected synchronized void write(SelectionKey key) throws IOException {
		SocketChannel socketChannel = (SocketChannel) key.channel();
