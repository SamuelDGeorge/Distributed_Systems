...
public class BroadcastingNioServer implements Runnable {
	...
	public BroadcastingNioServer(InetAddress hostAddress, int port) throws IOException {
	...
	public void processData(BroadcastingNioServer server, List<SocketChannel> sockets, byte[] data, int count) {
		byte[] dataCopy = new byte[count];
		System.arraycopy(data, 0, dataCopy, 0, count);
		for(int i = 0; i < sockets.size(); i++) {
			try {
				SocketChannel currentChannel = this.openSockets.get(i);
				if (currentChannel.isConnected()) {
					server.send(currentChannel, dataCopy);
				}
			} catch (Exception e) {}
		}
	}
